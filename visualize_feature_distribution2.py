...
# 假设我们有一个特征提取器和一个标签预测器
feature_extractor = FeatureExtractor()
label_predictor = LabelPredictor()

# 我们还需要一个损失函数和一个优化器
criterion = nn.CrossEntropyLoss()
optimizer = paddle.optimizer.Adam(parameters=feature_extractor.parameters() + label_predictor.parameters())
...
# 使用你的数据和模型调用可视化函数
visualize_feature_distribution(real_images_loader, hand_drawn_images_loader, feature_extractor)# 最好是feature_extractor_without_dat