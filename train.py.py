...
def train_epoch(source_dataloader, target_dataloader, lamb):
    ...
    for i, ((source_data, source_label), (target_data, _)) in enumerate(zip(source_dataloader, target_dataloader)):
        source_data = source_data.cpu() #cpu or cuda
        source_label = source_label.cpu() #cpu or cuda
        target_data = target_data.cpu() #cpu or cuda

        # 我們把source data和target data混在一起，否則batch_norm可能會算錯 (兩邊的data的mean/var不太一樣)
        mixed_data = torch.cat([source_data, target_data], dim=0)
        domain_label = torch.zeros([source_data.shape[0] + target_data.shape[0], 1]).cpu() #cpu or cuda
